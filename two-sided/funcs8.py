...
def make_site_array(site_types, values, at_ids=None, number_of_replica=1, **kwargs):
...
def make_CTCF_arrays(site_types, CTCF_left_positions, CTCF_right_positions, CTCF_facestall, CTCF_backstall, **kwargs):
...
def make_CTCF_dynamic_arrays(site_types, CTCF_lifetime, CTCF_offtime, sites_per_monomer, velocity_multiplier, **kwargs):
...
def make_LEF_arrays(site_types, LEF_lifetime, LEF_stalled_lifetime, LEF_birth, LEF_pause, sites_per_monomer, velocity_multiplier, **kwargs):
...
def make_translocator_bak(extrusion_engine, site_types, CTCF_left_positions, CTCF_right_positions, **kwargs):
    ...
    # New params
    mode = kwargs.get('mode', 'asymmetric') 
    switchProb = kwargs.get('switchProb', 0)
    speedMultiplier = kwargs.get('speedMultiplier', 1.0) 
    ...
    # updated
    LEFTran = extrusion_engine(number_of_LEFs, *LEF_arrays, *CTCF_arrays, *CTCF_dynamic_arrays, mode=mode, switchProb=switchProb, speedMultiplier=speedMultiplier)
...
def make_translocator(extrusion_engine, site_types, CTCF_left_positions, CTCF_right_positions, **kwargs):
    LEF_separation = kwargs['LEF_separation']
    velocity_multiplier = kwargs['velocity_multiplier']
    sites_per_monomer = kwargs['sites_per_monomer']
    number_of_replica = kwargs['number_of_replica']
    ...
    switchProb = kwargs.get('switchProb', 0)
    speedMultiplier = kwargs.get('speedMultiplier', 1.0)
    stability_factor = kwargs.get('stability_factor', 0.1) # New params
    ...
    LEFTran = extrusion_engine(
        ...
        *CTCF_dynamic_arrays, 
        ...
        stability_factor=stability_factor # New params
    )
